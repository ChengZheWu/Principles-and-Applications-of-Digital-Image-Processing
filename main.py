...
class plot(QDialog):
    ...
    def img_generator(self, data):
        ...
        for line in data.split("\n"):
            row = []
            for char in line:
                num = self.convert(char)  # 將值轉換為 0-255
                if num != None:  # "\n"等其他字串會回傳 none
                    img.append(num)
        img = np.array(img, dtype=np.uint8).reshape(64, 64)  # reshape 圖片大小
        return img

    def add(self, img):
        """
        將影像的pixel value加上一個常數
        """
        img = img.astype(np.int32)  # 先轉換為32位元，以免 overflow
        img += 100
        img[img < 0] = 0
        img[img > 255] = 255
    ...
    def plot_histogram(self, img):
        ...
        plt.ylabel("num")  # 該 pixel value 的數量
        plt.show()
